...
public class ProcessingDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Tank>(tank =>
        {
            ...
            tank.Property(t => t.Kind).HasConversion<string>().HasMaxLength(20).IsRequired(); // varchar not ENUM per AC 57
            tank.Property(t => t.Status).HasConversion<string>().HasMaxLength(20).IsRequired();
            tank.Property(t => t.CapacityKg).HasPrecision(10, 2).IsRequired(); // explicit HasPrecision per DOD 57, KG not litres per user
            tank.Property(t => t.RemainingKg).HasPrecision(10, 2).IsRequired(); // stored and updated on each allocation/unload for next-day retrieval
            tank.Property(t => t.RowVersion).IsRowVersion().IsConcurrencyToken(); // concurrency token for concurrent-safe updates per AC 57
            tank.Property(t => t.CreatedAtUtc).HasColumnType("datetime(6)").IsRequired(); // UTC datetime(6) per AC
            ...
            // Seed 3 storing + 3 mixing tanks per DOD 57
            tank.HasData(
        ...
        modelBuilder.Entity<ProcessingRun>(run =>
        {
            ...
            run.Property(r => r.State).HasConversion<string>().HasMaxLength(50).IsRequired(); // varchar per AC 57
        ...
        modelBuilder.Entity<QualityPanel>(panel =>
        {
            ...
            panel.Property(p => p.FatPercent).HasPrecision(10, 2).IsRequired(); // 3.85 per AC example
            ...
            panel.Property(p => p.Snf).HasPrecision(10, 2).IsRequired(); // 8.64
            panel.Property(p => p.Ts).HasPrecision(10, 2).IsRequired(); // 12.49
            panel.Property(p => p.Ph).HasPrecision(10, 2).IsRequired(); // 6.72
            panel.Property(p => p.KqColour).HasMaxLength(20).IsRequired().HasConversion<string>(); // varchar
            panel.Property(p => p.AlcoholOutcomesJson).HasMaxLength(1000).IsRequired();
            panel.Property(p => p.AlcoholResult).HasMaxLength(50).IsRequired(); // varchar per AC
            panel.Property(p => p.Verdict).HasMaxLength(20).IsRequired().HasConversion<string>(); // Accept/Reject varchar
        ...
        modelBuilder.Entity<TankAllocation>(alloc =>
        {
            ...
            alloc.Property(a => a.ProductType).HasConversion<string>().HasMaxLength(20).IsRequired(); // varchar SY,SK,FM,FLM,DK
            alloc.Property(a => a.BatchNumber).IsRequired(); // 1-365
            alloc.Property(a => a.BatchLetter).HasMaxLength(2).IsRequired(); // A-Z
            alloc.Property(a => a.BatchCode).HasMaxLength(20).IsRequired(); // e.g. 1-SY-A
            alloc.HasIndex(a => a.BatchCode).IsUnique().HasDatabaseName("ux_tank_allocations_batchcode");
            alloc.Property(a => a.AllocatedAtUtc).HasColumnType("datetime(6)").IsRequired(); // timestamp when batch created per user
            ...
            // Unique constraint for batch letter per day per product per DOD batch code spec: (BatchNumber, ProductType, BatchLetter) unique
        ...
        modelBuilder.Entity<ProcessingStage>(stage =>
        {
            ...
            stage.HasKey(s => s.Id);
            stage.Property(s => s.StageType).HasConversion<string>().HasMaxLength(20).IsRequired(); // varchar per AC 57, single table per DOD 65
            ...
            stage.Property(s => s.EndTemperatureC).HasPrecision(10, 2).IsRequired(); // 84.50 per AC example
